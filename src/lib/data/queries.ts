...
export async function getProductsByIds(ids: string[], limit = 8) {
  await connectDB();
  const cleaned = [...new Set(ids.map(String).filter((id) => /^[a-f\d]{24}$/i.test(id)))];
  ...
  const order = new Map(cleaned.map((id, i) => [id, i]));
  return [...products]
    .sort(
      (a, b) =>
...
export async function searchCollections(query: string, limit = 6) {
  await connectDB();
  const q = query.trim();
  if (!q) return [];
...
export async function instantSearch(query: string, productLimit = 6, collectionLimit = 4) {
  const q = query.trim();
  if (!q) {
    return { products: [], collections: [], totalProducts: 0 };
  }

  const [productResult, collections] = await Promise.all([
    getProducts({ search: q, limit: productLimit, page: 1 }),
    searchCollections(q, collectionLimit),
  ]);
